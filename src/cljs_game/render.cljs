...
;; (Yay closures!)
(defn ^:export create-sprite-component!
  ([image-url] (create-sprite-component! image-url nil nil))
...
(defrecord ^:export ThreeJSSprite [object texture atlas-key]
  ISprite
  (set-texture [this texture]
...
(defn ^:export sprite
  ...
  (let [material (three/SpriteMaterial. )
    ...
    (let [width (.-width (.-image texture))
      ...
      (set! (.-y (.-scale sprite)) height)
      (set! (.-needsUpdate material) true))
    (update entity :renders conj (->ThreeJSSprite sprite texture nil))))
